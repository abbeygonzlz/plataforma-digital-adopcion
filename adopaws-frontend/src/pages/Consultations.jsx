...
export default function Consultations() {
  ...
  useEffect(() => {
    ...
    userService.getAll().then(r => {
      ...
      setVets(all.filter(u => u.userType === 'vet' || u.userType === 'shelter'))
    }).catch(() => setVets([]))
              ...
              : vets.map((v, i) => (
                <div key={v.idUser ?? v.id ?? i} style={{ background: 'var(--color-surface)', border: '1px solid var(--color-border-light)', borderRadius: '16px', padding: '1.5rem', display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                  ...
                  <div>
                    ...
                    <p style={{ fontSize: '0.8rem', color: 'var(--color-text-muted)', margin: '2px 0 0' }}>{v.userType === 'vet' ? 'Veterinario' : 'Refugio'} · {v.region || 'Sin región'}</p>
                  </div>
