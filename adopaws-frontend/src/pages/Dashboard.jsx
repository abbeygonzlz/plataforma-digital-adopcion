      ...
      <form onSubmit={(e) => { e.preventDefault(); onSubmit(form) }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '0.85rem' }}>
          {[
            ...
            { name: 'region', label: 'Región', type: 'text', placeholder: 'Ej: Liberia' },
          ].map(f => (
            <div key={f.name}>
        ...
        <div style={{ marginTop: '0.85rem' }}>
          ...
          <textarea name="description" value={form.description} onChange={handleChange} rows={3}
            placeholder="Personalidad y historia de la mascota..."
...
export default function Dashboard() {
  ...
  const [requests, setRequests] = useState([]) // { petName, ...request }
  ...
  const loadRequests = async () => {
    ...
    try {
      const r = await shelterService.getMyPets()
      const list = Array.isArray(r.data) ? r.data : []
      // Por cada mascota, traer sus solicitudes
  ...
  const handleCreate = async (form) => {
    ...
    try {
      ...
      const res = await petService.create({ ...petData, age: parseInt(form.age) || 0, idUser: user.id })
